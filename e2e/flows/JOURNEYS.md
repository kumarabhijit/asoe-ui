...
### Per-archetype flow lists (auto-generated)

The matrix above slices by flow → (journey, arc). The block below
slices the same data by archetype → flows so each archetype's
ownership is visible at a glance. Generated by the same
`npm run flows:matrix` command; drift between this block and the
flow YAMLs fails the journey-matrix vitest drift test.
...
- `resolve/exception-triage-approval.yaml` — task-completion
- `signout/signout-from-each-role.yaml` — orientation
- `triage/inbox-item-click-behavior-catalog.yaml` — orientation
- `triage/inbox-load.yaml` — orientation
