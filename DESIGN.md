...
│   ├── ProvenanceCard.tsx        # Diagnostics & Audit "Provenance" key/value projector (presentation.audit)
...
│   ├── useExceptionActions.ts    # HITL handlers (approve / reject / escalate / override / cosign / reanalyze); ADR-045 CP3 — escalate + cosign capture reasons via in-panel dialogs (no window.prompt)
...
| `ConfidenceDisplay` | Tailwind | Canonical cross-case confidence renderer. Normalises any producer's score to canonical [0,1] via `src/lib/confidence.ts` (`scale="unit"` for 0–1 fields, `"percent"` for 0–100), maps to a visual band (`high`/`review`/`low` — visual mapping, not a decision threshold), and frames the value honestly: a raw model score unless the backend reports `calibrated` (the calibration claim is itself audit-bearing). Variants: `bar` (Layer-1 card), `inline` (chip), `prominent` (hero + raw-float audit fidelity). Band is never colour-only — value + band + calibration posture are in the `aria-label` (WCAG 1.4.1). Centralisation is source-grep-locked by `tests/architectural/confidence_centralized.test.ts`. | AgentReasoningCard, EmailOrderEntrySection, DuplicateDetectionSection, EntitiesSection, OrderEntrySection |
| `ReviewQualityPanel` | Tailwind + MetricTile | Autonomy program's real-data slice. Projects the backend automation-bias SLIs (`GET /api/v1/metrics/reviewer-activity`) — evidence-scrutiny rate, scrutiny-by-highlight cohort (the ADR-043 §2.7 regression signal), decision-dwell distribution — the "reviewing vs rubber-stamping" gate for safe autonomy. Counterfactual STP + calibration reliability (no request-time source yet) render as explicit "no data source yet" cards, never fabricated charts. Mounted by `/settings/autonomy` (manager+/admin, gated on `exceptions:override`). Deliverable-locked by `tests/architectural/autonomy_review_quality_deliverable.test.ts`. | `/settings/autonomy` |
| `ProvenanceCard` | Tailwind + EvidenceBlock + Badge | Diagnostics & Audit "Provenance" key/value card (cockpit / Modern view, council 2026-06-10). Dumb projector of `analysis.presentation.audit` — recipe, raw intent, originating event, shadow verdict, taxonomy classification + version, trace correlation id. Each row routes through `EvidenceBlock` (contextual) so an absent field is structurally omitted (no `—`, no fabricated value); an empty bundle renders nothing. `presentation_tier: audit` — drawer only, never Layer 1 (Guardrail #0). Locked by `tests/architectural/cockpit_wiring.test.ts`. | ExceptionDetailPanel (Diagnostics & Audit tier) |
...
enrichment sections → `EvidenceGrid` → `DiagnosticsSection`. The
...
sit at the top of the viewport, and the recipe-authoritative comparison
auto-expands via the backend `primary_section` hint (Priority-2).
(A short-lived experiment that moved Diagnostics into a tab was reverted
per PO ruling 2026-06-07 — `EvidenceGrid` and `DiagnosticsSection` stay
inline collapsed sections, the operator-preferred layout.) Pre-S15a it had its
...
**S1 dedup — `embedded` prop:** When `ExceptionDetailPanel` mounts inside `CaseDetailPanel` (the `/cases` workspace path), the `embedded` prop forwards into `HeaderRibbon` (drops breadcrumb + Audit Result badge) and `ContextStrip` (drops Customer row) so the per-record detail does not duplicate the case-level chrome above it. The standalone `/exceptions/[id]` path keeps full chrome. A new `SectionAnchorBar` (sticky nav with Recommendation / Evidence / Diagnostics anchors) renders between `ContextStrip` and the sticky action ribbon so operators don't linearly scroll through 5–8 sections to reach the bottom.

| Layer | Sub-Component | File | Purpose |
|---|---|---|---|
| 1 | `HeaderRibbon` | `HeaderRibbon.tsx` | Breadcrumb context, lifecycle/verdict badges, total value. In Modern, when the Situation sub-line carries the lifecycle state, the `suppressLifecycle` prop relocates the "Current State" badge into the sub-line (one fact, one home); embedded + relocated renders nothing. |
| 1 | `SituationSubline` | `SituationSubline.tsx` | Modern-only decision-context line under the Situation headline (audit finding #2, option C — sign-off 2026-06-10): SLA (live relative label via `lib/sla.ts` + `useSlaTicker`, from the backend-composed `presentation.situation_context.sla_due_at` timestamp) + lifecycle state (governed humanizer). Dumb projector — sub-line membership is backend-owned; null facts structurally omitted; the $ figure stays in `ImpactBar`. |
...
**Override chooser dialog (Phase 3):** The `Override…` button on `AgentReasoningCard` opens an override chooser dialog rendered inline in `ExceptionDetailPanel.tsx`. The dialog sources its resolution-action options from `health.allowed_resolution_actions` (preferring a record-specific narrower list from `resolution_data.allowed_actions` when the server supplies one), and its reason-category options from `health.allowed_override_reason_tags_by_intent[detail.intent]` (falling back to the global `health.allowed_override_reason_tags`). Notes are mandatory (SOX). Submission calls `exceptionsApi.disposition()` with `{ action, notes, reason_tag }`. No free-text action input — Guardrail #2. **Both pickers are typeahead Comboboxes** (S2 follow-up #6 — `src/components/ui/Combobox.tsx`); cluster grouping (ADR-033 §D) is preserved via the `groups` prop.

**Mandatory `reason_tag` on YELLOW/RED Approve/Reject (S2 #7):** Approve and Reject on a YELLOW or RED record now require an operator-picked `reason_tag` from the same vocabulary the Override dialog uses. The comment-swap inside `ActionButtonMatrix` renders a native `<select>` above the textarea; Confirm Approval / Confirm Rejection stay disabled until a tag is picked, and `Cmd+Enter` cannot bypass the gate. GREEN keeps the optional-comment one-step path. The mandatory tag is the structured signal ASOE's recipe-calibration loop joins on — free-text comments alone produced no categorical training data.

**Four-eyes cosign banner:** When `detail.lifecycle_state === "PENDING_COSIGN"`, `ExceptionDetailPanel` renders a cosign banner above the AgentReasoningCard showing initiator, staged action, reason_tag, and financial impact (from `resolution_data.pending_override`). Non-initiator manager+ users see `[Approve cosign] [Reject cosign]` buttons; clicking opens an in-panel notes dialog (ADR-045 CP3 — `openCosign(approve)` arms it, `submitCosign()` calls `exceptionsApi.cosign()`). Notes are mandatory (SOX); `window.prompt` was removed. The initiator sees a read-only "awaiting cosign" message (SoD enforcement mirrored from backend). `ActionInFlight` extends with `cosign-approve` / `cosign-reject` for pessimistic UI.

**Single action host + uniform reason capture (ADR-045 CP3):** The action buttons live in exactly one place — the Agent Recommendation pane (`AgentReasoningCard`), which always renders `ActionButtonMatrix`. The separate `StickyActionRibbon` was retired; the above-the-fold guarantee is preserved by keeping `AgentAnalysisSection` collapsed by default and auto-expanding it only for HITL states. Every state-changing action captures its mandatory reason/notes through the same constrained in-panel dialog (Approve/Reject comment + reason_tag; Reanalyze and Escalate a mandatory free-text reason; Cosign mandatory notes) — no `window.prompt` anywhere. Every disabled action button carries a `title` explaining why ("disabled is never silent"). Locked by `tests/architectural/action_reason_capture_guard.test.ts` and `tests/architectural/exception_detail_panel_section_reorder.test.ts`.
...
**Action ribbon hotkeys (S2 #3):** `ActionButtonMatrix` registers `A` (Approve), `R` (Reject), `O` (Override…), `E` (Escalate), `Y` (Re-analyze) via `useHotkeys`. Bindings are gated by the same predicates that hide the visible buttons — a hidden button cannot fire from the keyboard. The hotkey registry at `src/lib/hotkeys.ts` is the single source of truth that both the listener AND the `?` cheatsheet read from. Each ribbon button carries a small aria-hidden `<kbd>` suffix (S2 #13) reinforcing the binding letter; the SR contract is on the existing `aria-keyshortcuts`.

**Next-case auto-advance (S2 #11):** `handleRecordActionComplete` on `/cases/page.tsx` snapshots the next case in the visible queue BEFORE `refetch()`/`refreshCaseDetail()` and `router.replace`s to it. Last case in queue is a no-op. Reanalyze follows the same path. Source-locked at `tests/architectural/s2_tier3_audit_locks.test.ts`.

**`?` keyboard cheatsheet (S2 #5):** `HotkeyCheatsheet` mounted once on `/cases/page.tsx`. `?` (Shift+/) toggles; Escape closes. Renders every entry in the hotkey registry grouped by scope (global / queue / picker / ribbon / comment).

**A11y:** `role="listbox"` on the queue, `role="option"` on each row with `aria-selected`. `tabIndex={0}` on the listbox so `:focus-visible` tracks the keyboard cursor. The slim context strip uses `aria-label="Case context"`; the full header uses `aria-label="Case header"`. `SlaBandAnnouncer` (S3 #B) emits a polite live-region message when the selected case crosses a band; the per-minute ticker re-renders are silenced via `aria-live="off"` on the `Last activity` spans (S3 #E). Custom-dialog focus restore (`useFocusRestoreOnClose`, S3 #C) wraps `HotkeyCheatsheet` + the `ActionButtonMatrix` comment swap; Radix Dialog's built-in restore covers `OverrideChooserDialog`.

**Telemetry (S2 #8):** `reportTimeToActionSubmit({case_id, dwell_ms, action, reason_tag?})` fires from `ActionButtonMatrix.confirmAction` via `useCaseTelemetry.markActionSubmit`. Distinct from the existing `time-to-first-action` (which fires on the first click) — the submit metric captures end-to-end CSA time including comment + tag-pick dwell and carries the operator-picked reason_tag for calibration joins.
