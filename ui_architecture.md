...
| D18 | `OrderAnalysis` enrichment fields — adapter-by-adapter backend backing | Cross-repo review L2 / H5 flagged enrichment sections rendering from mock-only fields; the real backend `AnalysisResponse` (asoe2/api/schemas.py) originally carried only `diagnosis / confidence / risk / resolution / lines[]`. Resolution is underway via the adapter registry at `asoe2/api/analysis_adapters.py::ANALYSIS_ADAPTERS` — one adapter per enrichment field, projecting `record.resolution_data` (or a synthetic recipe invocation for shadow-gated records) into a typed Pydantic model. **Backend-backed today:** `price_hold_analysis` (adapt_price_hold), `edi_mismatch_analysis` (adapt_edi_mismatch). **Still mock-only:** `duplicate_detection`, `order_comparison`, `price_analysis`, `backorder_analysis`, `overmax_analysis`, `moq_analysis`, `pallet_analysis`, `delivery_delay_analysis`. Each mock-only field carries a `// preview-only` marker on its `OrderAnalysis` entry; the marker is dropped per-field as the matching adapter lands. The two Playwright specs (`tests/browser/price-hold-detail.spec.ts`, `edi-mismatch-detail.spec.ts`) are no longer `test.skip` — they exercise the two shipped adapters end-to-end. Linked to the `NEXT_PUBLIC_SHOW_PREVIEW_INTENTS` backlog pattern in tasks.md. | **PARTIAL** — 2 of 10 adapters shipped (2026-04-22); 8 remaining tracked as L2d follow-up |
