...
| D18 | `OrderAnalysis` enrichment fields — adapter-by-adapter backend backing | Cross-repo review L2 / H5 flagged enrichment sections rendering from mock-only fields; the real backend `AnalysisResponse` originally carried only `diagnosis / confidence / risk / resolution / lines[]`. The Verdict workshop (2026-04-22) prescribed a three-pillar fix; all three landed: (1) `GraphState.enrichment_context` persisted (Pillar 1); (2) `build_analysis` graph node enforces the `compliance/audit_bearing_registry.yaml` and emits `TerminalStatus.AUDIT_CONTEXT_MISSING` with structured trace fields when coverage fails (Pillar 2); (3) `EvidenceBlock` UI primitive enforces structural omission OR "Context Not Required for Resolution" instead of dashes (Pillar 3). **Backend-backed today (6 of 10):** `price_hold_analysis`, `edi_mismatch_analysis`, `delivery_delay_analysis`, `overmax_analysis`, `moq_analysis`, `pallet_analysis`. **Still mock-only (4 of 10):** `duplicate_detection`, `order_comparison`, `price_analysis`, `backorder_analysis` — all gated on gateway-persistence work (matched_po_details, warehouse snapshots, contract refs) that requires upstream schema changes beyond the adapter pattern. Each backend-backed section's UI type tier-aligned: audit-bearing required, grandfathered audit-bearing optional with named clauses (`overmax_gateway_gap`, `moq_gateway_gap`, `delivery_delay_financial_gap`, `price_analysis_gateway_gap`), conditional uses `useConditionalField` + EvidenceBlock predicate. Mock-only fields keep their `// preview-only` markers. | **PARTIAL** — 6 of 10 adapters shipped (2026-04-22); 4 remaining gated on gateway persistence (consolidated backlog C1-C7) |
