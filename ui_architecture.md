...
| D18 | `OrderAnalysis` enrichment fields are mock-only | Cross-repo review H5 flagged `DeliveryDelaySection` rendering from a mock-only field; the gap is wider — the real backend `AnalysisResponse` (asoe2/api/schemas.py:436) carries only `diagnosis / confidence / risk / resolution / lines[]`, so ALL 10 `OrderAnalysis` enrichment fields (`duplicate_detection`, `order_comparison`, `price_analysis`, `backorder_analysis`, `overmax_analysis`, `moq_analysis`, `pallet_analysis`, `delivery_delay_analysis`, `price_hold_analysis`, `edi_mismatch_analysis`) are populated only by the mock `exceptionsApi.orderAnalysis()`. Real-backend mode collapses every enrichment section via the data-presence guard. Each field now carries a `// preview-only` marker in `src/types/exceptions.ts` and the mock `orderAnalysis` function has a PREVIEW-ONLY docstring naming the backend gap. Resolution path: asoe2 must promote the enrichment fields onto AnalysisResponse (field-by-field, or by wiring the trace-extracted analysis payload through). Linked to the `NEXT_PUBLIC_SHOW_PREVIEW_INTENTS` backlog pattern in tasks.md. | **OPEN** — tracked as asoe2/asoe-ui joint follow-up (2026-04-22) |
