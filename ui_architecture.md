...
- **Responsive collapse (current):** CSS grid `lg:grid-cols-[360px_minmax(0,1fr)]` ≥1024px, single column below. Finer-grained collapse rules (right overlays middle <1280px; middle becomes popover <1024px) are queued for a P3d-rest follow-on.

**URL state:** `?case=<caseId>` selects the case; `?record=<recordId>` selects the record. Both writes use `router.replace` so back/forward + reload preserve the cursor. Switching cases drops the prior `?record=` so the auto-mount picks the new case's first record.
