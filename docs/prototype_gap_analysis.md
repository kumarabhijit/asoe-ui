...
|---|---|---|
| 2.1 Top-level layout | 4 routes (`/exceptions`, `/dashboard`, `/inbox`, `/settings`) | 5 surfaces: `/home` (operational landing — PO #5/#6), `/cases` (canonical case workspace — ADR-041 P3 two-pane queue + inline detail; `/cases/[id]` for focused single-case deep-links), `/dashboard` (renamed "Performance"), `/settings`, `/inbox` (server redirect into `/cases?source=manual_order` — PO #9). `/exceptions` and `/exceptions/:path*` permanent-redirect to `/cases` (ADR-041 P2/P4). |
| 4.7 AI Intake Pipeline sub-view | Not built | Partially addressed — `/home` "Top of queue" + `useSlaTicker` countdown cover the SLA-tracking arm of the prototype's 6-step timeline. The full Received → Ingested → Classified → SAP Check → Routed → Resolved per-record view is still **not built** as a dedicated surface; the per-event view lives inside `CaseDetailPanel` (mounted at `/cases?case=<id>&record=<rid>` or `/cases/<id>?record=<rid>`) with the orchestration `PipelineDAG` rendered by the inline `DiagnosticsSection`. |
