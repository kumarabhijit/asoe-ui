...
export default function ExceptionDetailPanel({ exceptionId, onActionComplete, onRefreshRef }: ExceptionDetailPanelProps) {
  ...
  /* ── Actions (persona-gated) ──────────────────────────────────────── */

  async function handleApprove(comment: string) {
    if (!canDo("can_run_agents")) {
      addToast("warning", "Permission denied: your persona cannot approve exceptions.");
      return;
    }
  ...
  async function handleReject(comment: string) {
    if (!canDo("can_run_agents")) {
      addToast("warning", "Permission denied: your persona cannot reject exceptions.");
      return;
    }
    setActionLoading(true);
    try {
  ...
  async function handleEscalate() {
    if (!canDo("can_run_agents")) {
      addToast("warning", "Permission denied: your persona cannot escalate exceptions.");
      return;
    }
    setActionLoading(true);
    try {
