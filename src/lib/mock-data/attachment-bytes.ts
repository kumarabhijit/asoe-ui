...
// General (non-evidence) boilerplate wrapped around the evidence lines in the
// simple top-down mock PDF, so highlighted evidence is visually discriminable
// from ordinary document content. These strings must NEVER contain an evidence
// span (a duplicate flips the safety bar to AMBIGUOUS) — locked by
// tests/lib/attachment_bytes.test.ts.
const GENERAL_PDF_HEADER = [
  "This document was generated by the customer order desk.",
  "Dear team,",
  "Please find the relevant order details below for your review.",
] as const;

const GENERAL_PDF_FOOTER = [
 ...
 *  Non-geometry evidence (e.g. the From: header) stacks in a labelled band
 *  below the box band. General document text (letterhead, terms, footer)
 *  carries NO evidence span — it exists so a highlight is visually
 *  discriminable from ordinary content (a document that contains only
 *  evidence makes every line look highlighted). */
function makePo8842Pdf(evidenceText: string[]): string {
  const runs: PositionedRun[] = [
    ...
    // Letterhead block (left) + commercial terms (right column, clear of the
    // bbox band at x < 0.52·W) + footer boilerplate. None of these strings may
    // contain an evidence span — see the lock in tests/lib/attachment_bytes.
    { text: "Acme Beverages Co - Vendor VENDOR-7788", x: 64, y: PAGE_H - 80, size: 10 },
    { text: "Buyer: Southeast Beverage Distributors", x: 64, y: PAGE_H - 96, size: 10 },
    { text: "Terms: Net 30", x: 360, y: 660, size: 10 },
    { text: "Currency: USD", x: 360, y: 642, size: 10 },
    { text: "Incoterms: FOB Origin", x: 360, y: 624, size: 10 },
    { text: "Standard supply agreement applies to this order.", x: 64, y: 132, size: 10 },
    { text: "Page 1 of 1", x: 64, y: 112, size: 10 },
  ];
  ...
  // The band label is general text, not evidence.
  let fy = PAGE_H * (1 - 0.52);
  const footerEvidence = evidenceText.filter((t) => !placed.has(t));
  if (footerEvidence.length > 0) {
    runs.push({ text: "Email context:", x: 64, y: fy, size: 10 });
    fy -= 18;
  }
  for (const t of footerEvidence) {
...
export function mockAttachmentBlob(opts: {
  ...
  if (isPdf) {
    ...
    // mock PDF uses the simple top-down layout. Both wrap the evidence in
    // GENERAL (non-evidence) text so a highlight is visually discriminable
    // from ordinary document content.
