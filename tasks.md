...
- [x] **Autonomy v2 gating-ladder migration (UI mirror)** — `EdiMismatchAnalysisData.autonomy_level` widened to `L1|L2|L3|L4` (SHIP_TO_MISMATCH → L4 escalate under v2) with generated.ts round-trip; `cases.ts` transition fallback labels mirror the backend v2 vocabulary; `autonomy_union_parity` lock updated
- [x] **ADR-042 → Accepted** (2026-05-25) — autonomy-v2 dual-control compliance sign-off landed; backend gating ladder migrated to vocab v2, the UI already renders v2 labels from `health.allowed_autonomy_levels`, so the operator vocabulary and the engine agree under one version

## PARITY — Azure pre-prod ↔ Vercel dev parity (UI track)

Tracks the UI-side of `asoe2/docs/plans/azure-preprod-parity-plan.md`.
Cross-repo source-of-truth status table lives at the top of that file.
